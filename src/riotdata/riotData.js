...
for (var champName in champions['data']) {
    // skip loop if the property is from prototype
    if (!champions['data'].hasOwnProperty(champName)) continue;

    champions['data'][champName.toLowerCase()] = champions['data'][champName];
    delete champions['data'][champName];
}

var champsById = {};
for (var champName in champions['data']) {
    champsById[champions['data'][champName].id] = champions['data'][champName];
}

exports.champsById = champsById;
...
var items = {
  ...
  "data": {
    ...
    "3146": {
      ...
      "description": "<stats>+40 Attack Damage<br>+80 Ability Power</stats><br><br><unique>UNIQUE Passive:</unique> Heal for 15% of damage dealt. This is 33% as effective for Area of Effect damage.<br><active>UNIQUE Active - Lightning Bolt:</active> Deals <levelScale>175 - 250</levelScale> (+30% of Ability Power) magic damage and slows the target champion's Movement Speed by 40% for 2 seconds (40 second cooldown, shared with other <font color='#9999FF'><a href='itembolt'>Hextech</a></font> items).",
...
for (var itemId in items['data']) {
  var itemName = items['data'][itemId].name;
  if(itemName) {
    itemName = itemName.toLowerCase();
  }
  itemsByName[itemName] = items['data'][itemId];
}

exports.itemsByName = itemsByName;
exports.items = items;
console.log('riotData loaded');